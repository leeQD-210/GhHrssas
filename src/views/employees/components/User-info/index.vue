<template>
  <div class="user-info">
    <el-row type="flex" justify="end" style="height:20px">
      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="工号">
            <el-input v-model="userInfo.workNumber" class="nputW" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
      ...
      <el-row class="inline-info">
        <el-col :span="12">
          <el-form-item label="员工头像">
            <component :is="uploadPics" ref="user_avatar" />
          </el-form-item>
        </el-col>
      </el-row>
      <!-- 保存个人信息 -->
    ...
    <el-form label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <!-- 放置上传图片 -->
          <component :is="uploadPics" ref="staffPhoto" />
        </el-form-item>
...
import { getEmployeeDetail, editEmployeeDetail, getPersonalDetail, updatePersonalDetail } from '@/api/employees'
export default {
  data() {
    return {
  ...
  methods: {
    async initData() {
      ...
      /* 初始化时将图片地址赋值给子组件，并做上标记已上传 */
      if (this.userInfo.staffPhoto && this.userInfo.staffPhoto.trim() !== '') {
        console.log(1111)
        this.$refs.user_avatar.fileList = [{ url: this.userInfo.staffPhoto, upload: true }]
      }
    ...
    async saveUser() {
      const fileList = this.$refs.user_avatar.fileList
      /* 查找没有上传完成的item */
      if (fileList.length > 0 && fileList.some(item => !item.upload)) {
        this.$message.error('请等待图片上传成功')
        return
      }
      await editEmployeeDetail({ ...this.userInfo, staffPhoto: fileList && fileList.length > 0 ? fileList[0].url : ' ' })
      this.$message.success('员工基础信息修改成功')
    },
    async savePersonal() {
      ...
      await updatePersonalDetail({ ...this.formData, staffPhoto: fileList && fileList.length > 0 ? fileList[0].url : ' ' })
      this.$message.success('员工个人信息修改成功')
    }
  }
}
</script>
