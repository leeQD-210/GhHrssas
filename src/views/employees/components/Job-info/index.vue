      ...
      <div class="block">
        ...
        <el-form-item label="HRBP">
          <el-select v-model="formData.hrbp" filterable placeholder="请选择" class="inputW">
            <el-option v-for="item in users" :key="item.id" :label="item.username" :value="item.id" class="inputW" />
          </el-select>
        </el-form-item>
...
import { getEmployeeSimple, getUserJobDetail, updataUserJobDetail } from '@/api/employees'
export default {
  data() {
    return {
  ...
  methods: {
    async initData() {
      this.formData = await getUserJobDetail(this.userId)
      this.users = await getEmployeeSimple()
    },
    async saveJob() {
      await updataUserJobDetail(this.formData)
      this.$message.success('员工岗位信息修改成功')
    }
  }
}
</script>
